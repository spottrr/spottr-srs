...
Both Investigators and Facility Operators have access to all features (upload, search, face matching, export). They differ in their priorities, not their permissions.
...
\begin{enumerate}
    ...
    \item As an \textbf{Investigator}, I want to upload a reference photo of a specific individual and find every frame where they appear so that I can build a timeline of their activity using a person I am authorized to search for.
...
\begin{tabular}{|l|p{10cm}|}
\hline
\textbf{Use Case}       & UC-3: Watchlist-Based Face Search \\ \hline
\textbf{Primary Actor}   & User \\ \hline
\textbf{Preconditions}   & Video footage has been uploaded and processed. The user has uploaded one or more reference photos to a watchlist. \\ \hline
\textbf{Postconditions}  & The user receives a chronological list of frames where watchlist individuals were detected. \\ \hline
\textbf{Main Success Scenario} &
    \begin{enumerate}[nosep, leftmargin=*]
        \item The user navigates to the watchlist page and uploads one or more reference photos of individuals they are authorized to search for.
        \item The system extracts face embeddings from the reference photos and stores them in the watchlist.
        \item The user selects a watchlist entry.
        \item The system retrieves all frames where that individual was detected by matching against the watchlist embedding.
...
\textbf{Alternative Flows} &
    \begin{itemize}[nosep, leftmargin=*]
        \item \textit{No matches found:} The watchlist entry returned no detections in the indexed footage.
    \end{itemize} \\ \hline
\end{tabular}
\caption{UC-3: Watchlist-Based Face Search}
\label{tab:uc3}
\end{table}
...
\textbf{Main Success Scenario} &
    \begin{enumerate}[nosep, leftmargin=*]
        ...
        \item The background job extracts keyframes at 1 FPS, runs SAM 2 segmentation and tracking \cite{ravi2024sam2}, SigLIP2 encoding \cite{zhai2025siglip2}, rule-based deduplication (see Section~\ref{subsubsec:dedup}), and face detection \cite{deng2019arcface} on each frame.
...
\textbf{Main Success Scenario} &
    \begin{enumerate}[nosep, leftmargin=*]
        ...
        \item The user enters a text query, uploads a reference image, or selects a watchlist entry.
...
\begin{itemize}
    ...
    \item UC-3: Watchlist-based face search (User)
...
\subsection{Watchlist Page}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{assets/mockup/faces-page.png}
    \caption{Watchlist page showing reference individuals uploaded by the user with appearance counts across indexed footage.}
    \label{fig:ui-faces}
\end{figure}

The watchlist page displays reference individuals the user has uploaded for face search. Each entry shows the reference photo, a label assigned by the user, the number of detected appearances across uploaded footage, and how many videos contain matches. Users can add new watchlist entries by uploading reference photos, remove entries, and select an entry to retrieve all matching frames. The system does not display or cluster faces of bystanders or other individuals not on the watchlist.
